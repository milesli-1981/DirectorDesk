...
- 组 / Group Dynamics（引力场，Baseline §58，最小原型）
  ...
  - 行进避障（过地形）：编队在行进 / 变阵途中遇到静态障碍时，**只让被挡的队员局部让位**，未挡的完全保持原编队位置；不再把整队塌缩成单列。让位在**队伍局部坐标系**里决策，一律沿「侧向」推开、**前向坐标不变**，因此队员不会被往后推、不丢前进进度（不会"放弃原路线"）。
    - **小障碍 → 分流贴两边过**：障碍窄到编队能「骑」过去（左右两端都伸到障碍轮廓外）时，槽位在障碍中心左侧的队员贴左沿、右侧的贴右沿，像水流绕石，过完自然合拢回编队。
    - **大障碍 / 长墙 → 整队绕同一侧**：无法分流时全队走远离障碍中心的一侧；侧移距离过大（>6m）则回退到世界轴最小穿透推出，保证不穿模。
    - 侧别由**编队槽位的侧向偏移**判定（时间的纯函数），故求解器仍无状态、可确定性复算、不会左右抖动。避让位移直接叠加在最终期望位上、不经惯性低通，避免被甩动抵消而「看着让了却仍穿过去」。
